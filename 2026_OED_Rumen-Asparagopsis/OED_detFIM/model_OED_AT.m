...
% Muñoz-Tamayo 2026
% input program for IDEAS 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Program to define the mathematical model
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   This part was automatically generated. Do not change

%   Symbolic definition of the parameters

syms k_Ibr p1 p2 p3 p4 

parameters = [ k_Ibr p1 p2 p3 p4 ];

%   Symbolic definition of the state variables

syms Z_ndf Z_nsc Z_pro S_su S_aa S_ac S_bu S_pr S_IN S_IC S_h2 S_ch4 X_su X_aa X_h2 Sg_co2 Sg_h2 Sg_ch4 S_br 

Xx = [ Z_ndf Z_nsc Z_pro S_su S_aa S_ac S_bu S_pr S_IN S_IC S_h2 S_ch4 X_su X_aa X_h2 Sg_co2 Sg_h2 Sg_ch4 S_br ];

%   Symbolic definition of the outputs

syms S_ac S_bu S_pr qg_co2 qg_h2 qg_ch4 S_br 

Yy = [ S_ac S_bu S_pr qg_co2 qg_h2 qg_ch4 S_br ];

%   Symbolic definition of the model inputs
...
%   In this part the user defines the model. 
%   Follow the structure below 
 %   Do not change the names of the items:   
 %   parameters   
 %   F   
 %   H   

 % Step 1 : Write the constant parameters of the model, e.g.
	 % Kh = 10; 

 % Step 2 : Write the functions of the model, e.g.
	 % ro = mumax * Xx(1)*Xx(2) / (K+Xx(1));   
...
%  The mathematical model is represented as follows : 
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		 ...
		 % x has a dimension of n
		 % F has a dimension of n
		 % y has a dimension of q
		 % H has a dimension of q
      ...
      K_su    = 9e-3;        % Monod constant associated with the utilization of sugars, mol/L  
      ...
      K_aa    = 6.4e-3;      % Monod constant associated with the utilization of amino acids; mol/L 
      ...
      K_h2    = 5.84e-6;     % Monod constant associated with the utilization of hydrogen, mol/L
      ...
      lambda_1 = p1-p2*pgas_h2 ;%Molar fraction of the sugars utilized via reaction 1 
      lambda_2 = p3+p4*pgas_h2 ;%Molar fraction of the sugars utilized via reaction 2      
      ...
      I_br = k_Ibr/(s_br+k_Ibr) ;%Inhibition factor of the methanogens growth rate by the action of bromoform 
      
      
      rho_su = km_su*s_su*x_su*I_H2*I_IN_lim/(K_su + s_su) ;%Utilization rate of sugars with H2 regulation, mol/(L h)
      rho_aa = km_aa*s_aa*x_aa/(K_aa + s_aa) ;%Utilization rate of amino acids, mol/(L h)
      rho_h2 = I_br*km_h2*s_h2*x_h2*I_IN_lim/(K_h2 + s_h2) ;%Utilization rate of hydrogen with bromoform inhibition, mol/(L h) 
      ...
      wNDF = 38.7; % fraction of NDF in the feed, %
      wPRO = 16;   % fraction of Protein in the feed, %
      wNSC = 39.7; % fraction of NSC in the feed, %
      ...
      qg_co2 = q_g*sg_co2 ;%mol/h
      qg_h2  = q_g*sg_h2  ;%mol/h 
      qg_ch4 = q_g*sg_ch4 ;%mol/h  
      
      % ---- State equations -----
      %dx/dt
      
      Z_ndf_t = (F_ndf_l_in - rho_ndf -F_ndf_out)*V_l ;%Neutral detergent fiber (NDF), g  
      Z_nsc_t = (F_nsc_l_in - rho_nsc + (fch_x*w_mb)*(rho_xsu + rho_xaa + rho_xh2) - F_nsc_out)*V_l ;%Non-structural carbohydrates (NSC), g 
      Z_pro_t = (F_pro_l_in-rho_pro + (fpro_x*w_mb)*(rho_xsu + rho_xaa + rho_xh2) - F_pro_out)*V_l ;%Proteins, g
      
      S_su_t = (rho_ndf/w_su + rho_nsc/w_su  - rho_su - F_su_out)*V_l ;%Sugars, mol 
      S_aa_t = (rho_pro/w_aa - rho_aa - F_aa_out)*V_l ;%Average amino acids, mol 
      
      S_ac_t = (Yac_su*rho_su + Yac_aa*rho_aa - F_ac_out)*V_l ;%Total acetate, mol 
      S_bu_t = (Ybu_su*rho_su + Ybu_aa*rho_aa - F_bu_out)*V_l ;%Total butyrate, mol 
      S_pr_t = (Ypr_su*rho_su + Ypr_aa*rho_aa - F_pr_out)*V_l ;%Total propionate, mol 
      
      S_IN_t = (YIN_su*rho_su + YIN_aa*rho_aa + YIN_h2*rho_h2 - F_IN_out)*V_l ;%Inorganic nitrogen, mol 
      S_IC_t = (YIC_su*rho_su + YIC_aa*rho_aa + YIC_h2*rho_h2 - rhoT_co2 - F_IC_out)*V_l ;%Inorganic carbon, mol 
      S_h2_t = (Yh2_su*rho_su + Yh2_aa*rho_aa - rho_h2 - rhoT_h2 - F_h2_out)*V_l ;%Hydrogen in the liquid phase, mol 
      S_ch4_t = (Ych4_h2*rho_h2 - rhoT_ch4 - F_ch4_out)*V_l ;%Methane in the liquid phase, mol 
      
      X_su_t = (Ysu*rho_su - rho_xsu - F_xsu_out)*V_l ;%Sugars-utilizing microbes, mol 
      X_aa_t = (Yaa*rho_aa - rho_xaa - F_xaa_out)*V_l ;%Amino acids-utilizing microbes, mol 
      X_h2_t = (Yh2*rho_h2 - rho_xh2 - F_xh2_out)*V_l ;%Hydrogen-utilizing microbes (methanogens), mol 
      
      Sg_co2_t = (V_l*rhoT_co2/V_g - F_co2g_out)*V_g ;%Carbon dioxide in the gas phase, mol 
      Sg_h2_t = (V_l*rhoT_h2/V_g  - F_h2g_out)*V_g ;%Hydrogen in the gas phase, mol 
      Sg_ch4_t = (V_l*rhoT_ch4/V_g - F_ch4g_out)*V_g ;%Methane in the gas phase, mol 
      S_br_t = (F_s_br_in - k_br*s_br - F_s_br_out)*V_l ;%Bromoform, g 

 % Step 3 : Write the elements of the vector F, e.g.
	 % F(1) = -ro/Y  - Kh*Xx(1)); 
 ...
 % Step 4 : Write the elements of the vector H, e.g.
	 % H(1) = ro; 
